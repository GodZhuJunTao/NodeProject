...
var checkReg = {
	...
	strongpaw:function(str) { //强密码(必须包含大小写字母和数字的组合，不能使用特殊字符，长度在6-16之间)
 ...
 */
function selectMenu(id,num) {
	var oBox = document.getElementById(id);
	var btn = oBox.children[0];
	var menu = oBox.children[1];
	//第一种款式,有动画效果的点击出现,点击离开
	if (num==1) {
		var istrue = true;
 ...
 */
function TabControl(id,active,num){
	var oBox=document.getElementById(id);
	var A=oBox.getElementsByTagName('a');
...
/*
	...
	参数一:请求一  :get  post
	参数二:接口
	参数三:数据(可选)   namme='tiantian'&psw=123456 传给后端的数据
	参数四:成功的回调函数(可选的)
 */
